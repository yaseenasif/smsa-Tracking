...
export class UpdateVehicleTypeComponent implements OnInit {
  ...
  getVehicleTypeById(){
    this.vehicleTypeService.getByIDVehicleType(this.vTID).subscribe((res:Vehicle)=>{
     this.vehicle=res
    },error=>{
   ...
   onSubmit() {
    this.vehicleTypeService.updateVehicleTypeById(this.vTID,this.vehicle).subscribe(res=>{
